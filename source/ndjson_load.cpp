...
std::string escape_whitespace(std::string_view string)
{
...
octave_value parse_json_value(simdjson::ondemand::value value) noexcept(false)
{
    ...
    case Type::string: {
        ...
        // get_string() method so a buffer that is repeatedly used may be better, this function is recursive
        // though, passing a string buffer repeatedly is kinda a hassle, maybe I will use std::stack instead
        // in the future so I can use iterate approach instead.
...
DEFUN_DLD(ndjson_load, args, , "ndjson_load(<ndjson_str>)")
{
    if (args.length() != 1) {
        octave::print_usage();
    }

    auto json_raw      = args(0).string_value();
    auto json_raw_size = json_raw.size();

    auto parser = simdjson::ondemand::parser{};
    auto stream = simdjson::ondemand::document_stream{};

    if (auto err = parser.iterate_many(json_raw).get(stream); err) {
    ...
    for (auto it = stream.begin(); it != stream.end(); ++it) {
        ...
        try {
            ...
            auto offset  = doc.current_location().value() - json_raw.data();
            offset      -= offset > 0;

            auto to_end = json_raw_size - static_cast<std::size_t>(offset);
            auto substr = escape_whitespace({ json_raw.data() + offset, std::min(to_end, 50ul) });
